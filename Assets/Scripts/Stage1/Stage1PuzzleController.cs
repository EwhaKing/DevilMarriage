...
/// 모든 경로를 붉게 밟은 뒤 시작 룬으로 돌아오면 클리어.
/// </summary>
public class Stage1PuzzleController : MonoBehaviour
{
    ...
    public bool CanMoveTo(RuneNode target)
    {
        ...
        return TryGetEdge(_currentRuneIndex, target.RuneIndex, out _);
    }

    public void TryMoveToRune(RuneNode target)
    {
        if (!CanMoveTo(target))
            return;
